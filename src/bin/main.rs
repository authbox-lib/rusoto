...
fn thing() {
    ...
    match s3_list_buckets_tests(&mut s3) {
        Ok(_) => {
            println!("Everything worked for S3 list buckets.");
        }
        Err(err) => {
            println!("Got error in s3 list buckets: {:#?}", err);
        }
    }
    ...
    match s3_create_bucket_test(&mut s3, &bucket_name, &region, None) {
        Ok(_) => {
            println!("Everything worked for S3 create bucket.");
        }
        Err(err) => {
            println!("Got error in s3 create bucket: {:#?}", err);
        }
    }
    ...
    match s3_get_object_test(&mut s3, &bucket_name) {
        ...
        Err(err) => {
            println!("Got error in s3 get object: {:#?}", err);
        }
    }
    ...
    match s3_delete_object_test(&mut s3, &bucket_name, "sample-credentials") {
        ...
        Err(err) => {
            println!("Got error in s3 delete object: {:#?}", err);
        }
    }
    ...
    match s3_put_object_with_reduced_redundancy_test(&mut s3, &bucket_name) {
        ...
        Err(err) => {
            println!("Got error in s3 put object with reduced redundancy: {:#?}",
                     err);
        }
    }
    ...
    // Set the file in s3_multipart_upload_test and uncomment this code to test
    // multipart upload:
    ...
    // Working example, replace bucket name, file name, uploadID for your multipart
    // upload:
    // match s3_list_multipart_upload_parts(&mut s3, &bucket_name, "testfile.zip",
    // "PeePB_uORK5f2AURP_SWcQ4NO1P1oqnGNNNFK3nhFfzMeksdvG7x7nFfH1qk7a3HSossNYB7t8Qh
    // cN1Fg6ax7AXbwvAKIZ9DilB4tUcpM7qyUEgkszN4iDmMvSaImGFK") {
    ...
    // Working example, replace bucket name, file name, uploadID for your multipart
    // upload:
    // match s3_abort_multipart_uploads(&mut s3, &bucket_name, "testfile.zip",
    // "W5J7SeEor1A3vcRMMUhAb.BKrMs68.
    // suzyhErssdb2HFAyDb4z7QhJBMyGkM_GSsoFqKJJLjbHcNSZTHa7MhTFJodewzcswshoDHd7mffXP
    // NUH.xoRWVXbkLjakTETaO") {
    ...
    match s3_delete_bucket_test(&mut s3, &bucket_name, &region) {
        Ok(_) => {
            println!("Everything worked for S3 delete bucket.");
        }
        Err(err) => {
            println!("Got error in s3 delete bucket: {:#?}", err);
        }
    }
    ...
    match s3_create_bucket_test(&mut s3,
                                &bucket_name,
                                &region,
                                Some(CannedAcl::AuthenticatedRead)) {
        Ok(_) => {
            println!("Everything worked for S3 create bucket with ACL.");
        }
        Err(err) => {
            println!("Got error in s3 create bucket: {:#?}", err);
        }
